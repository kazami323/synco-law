...
# Указание на чужую юрисдикцию СРАЗУ после названия акта: «ГК РФ»,
# «УК Российской Федерации», «ГК Казахстана», «ГК Республики Казахстан».
#
# В базе лежат только НПА Республики Узбекистан. Отдать на такой запрос
# узбекскую статью с тем же номером — худший вид ошибки: номер совпадает,
# предмет регулирования нет, а ответ выглядит корректно оформленным, вплоть
# до рабочей ссылки на lex.uz. Юрист процитирует чужой закон под верным
# номером и не заметит.
#
# Шаблон привязан к началу «хвоста» после акта, поэтому упоминание страны в
# другом месте вопроса («ст. 330 ГК — что с поставками в Россию?») ссылку не
# отменяет. «Республики <не Узбекистан>» ловится общим правилом, а не
# перечислением стран.
FOREIGN_JURISDICTION_AFTER_ACT = re.compile(
    r"^\W{0,3}(?:"
    r"рф|рк|рб|"
    r"росси\w*|российск\w*|"
    r"казахстан\w*|украин\w*|беларус\w*|белорусск\w*|"
    r"кыргыз\w*|киргиз\w*|таджикистан\w*|туркменистан\w*|"
    r"азербайджан\w*|армени\w*|грузи\w*|молдов\w*|"
    r"республик\w*\s+(?!узбекистан)\w+"
    r")\b",
    re.IGNORECASE,
)


def mentions_foreign_act(q: str) -> bool:
    """Запрос явно про акт чужого государства.

    Проверяется только текст непосредственно после названия акта: «ГК РФ» —
    да, «ГК» в вопросе про российского контрагента — нет.
    """
    for pattern, _title in ACT_REFERENCE_PATTERNS:
        match = pattern.search(q)
        if match is None:
            continue
        if FOREIGN_JURISDICTION_AFTER_ACT.search(q[match.end():]):
            return True
    return False
...
async def search_legal_articles(
    ...
    *,
    ...
    # Чужое право: лучше не найти ничего, чем подставить одноимённую по номеру
    # норму Узбекистана. Отсекается здесь, а не в _parse_legal_reference:
    # иначе запрос ушёл бы в поиск по словам и та же статья всплыла бы там.
    if mentions_foreign_act(q):
        return []
